...
public class UiFlowTest {
    ...
    @Test public void createWithPhotoRecreateSellAndRetainPhoto() throws Exception {
        ...
        Intent result=new Intent().setData(source);IntentFilter picker=new IntentFilter(Intent.ACTION_OPEN_DOCUMENT);picker.addCategory(Intent.CATEGORY_OPENABLE);picker.addDataType("image/*");Instrumentation.ActivityMonitor monitor=new Instrumentation.ActivityMonitor(picker,new Instrumentation.ActivityResult(Activity.RESULT_OK,result),true);instrumentation.addMonitor(monitor);
        ...
        screenshot("02-fiche-achat");click("save_article");waitFor(()->loaded().size()==1);waitFor(()->!jobPending());Item stock=loaded().get(0);assertEquals(3000,stock.cost());assertEquals(0,stock.profit());assertEquals(1,stock.photos.size());String savedPhoto=stock.photos.get(0);assertTrue(new PhotoStore(context).file(savedPhoto).isFile());context.getContentResolver().delete(source,null,null);
        ...
        scenario.onActivity(a->{TextView sell=findText(a.getWindow().getDecorView(),"Enregistrer la vente");assertNotNull(sell);sell.performClick();});instrumentation.waitForIdleSync();field("sale","45.00");field("salePlatform","Main propre");click("save_article");waitFor(()->loaded().get(0).sold);waitFor(()->!jobPending());Item sold=loaded().get(0);assertEquals(1500,sold.profit());assertEquals(savedPhoto,sold.photos.get(0));assertTrue(new PhotoStore(context).file(savedPhoto).isFile());
