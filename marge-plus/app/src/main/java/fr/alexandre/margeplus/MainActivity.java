...
public class MainActivity extends Activity {
    ...
    private void renderArticles(){
        ...
        HorizontalScrollView scroll=new HorizontalScrollView(this);scroll.setHorizontalScrollBarEnabled(false);LinearLayout chips=ui.row();String[] keys={"all","stock","sold"},names={"Tous","En stock","Vendus"};
    ...
    private void refreshResults(){
        ...
        for(Item i:items)if((filter.equals("all")||filter.equals("sold")==i.sold)&&(i.name+" "+i.category+" "+i.platform+" "+i.salePlatform).toLowerCase(Locale.FRANCE).contains(q))shown.add(i);
    ...
    private void renderDetail(){
        ...
        ui.add(body,ui.text(i.name,25,INK,true),7);ui.add(body,ui.text(join(i.category,i.platform),13,MUTED,false),12);ui.add(body,ui.chip(i.sold?"Vendu le "+displayDate(i.saleDate):"En stock depuis le "+displayDate(i.purchaseDate),i.sold?MINT:MUTED,i.sold?MINT_DARK:CARD2),18);
        LinearLayout hero=ui.card();hero.addView(ui.text(i.sold?"BÉNÉFICE NET":"COÛT TOTAL ENGAGÉ",11,MUTED,true));ui.gap(hero,8);long val=i.sold?i.profit():i.cost();hero.addView(ui.amount(Money.format(val),32,i.sold?(val>=0?MINT:RED):INK),new LinearLayout.LayoutParams(-1,ui.dp(42)));ui.gap(hero,6);hero.addView(ui.text(i.sold?"Vente − achat − tous les frais":"Achat et tous les frais saisis",12,MUTED,false));if(!i.sold&&i.estimate>0){ui.line(hero);kv(hero,"Revente envisagée",Money.format(i.estimate),INK);kv(hero,"Bénéfice estimé",Money.format(i.potentialProfit()),i.potentialProfit()>=0?MINT:RED);}ui.add(body,hero,16);
    ...
    private void renderTools(){
        ...
        ui.gap(body,14);ui.add(body,ui.text("MARGE +  ·  VERSION 2.0.0",11,MUTED,true),8);ui.add(body,ui.text("Cette application est distincte de Marge. L’ancienne application et ses données restent intactes. L’import de son export reprend le total des frais, car cet export ne contient pas leur détail.",12,MUTED,false),0);
    }
